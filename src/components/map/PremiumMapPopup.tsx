...
export const PremiumHoverPopup = React.forwardRef<HTMLDivElement, PremiumMapPopupProps>(
...
PremiumHoverPopup.displayName = 'PremiumHoverPopup';

export const PremiumDetailPanel = React.forwardRef<HTMLDivElement, PremiumMapPopupProps>(
  ({ feature, onClose, className }, ref) => {
    const props = feature.properties;
    const category = String(props.category || 'OTHER');
    const color = CATEGORY_COLORS[category] || '#3B82F6';
    const title = String(props.name || props.title || props.species || 'Unknown');
...
PremiumDetailPanel.displayName = 'PremiumDetailPanel';
